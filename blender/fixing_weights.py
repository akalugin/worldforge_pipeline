...
def has_alpha_diffuse_map(file_path):
    """Returns whether there exists a diffuse texture with an alpha channel at the specified file_path

    :file_path: The path to the file_path to check.
    :returns: Whether there exists a diffuse texture with an alpha channel at the specified file_path.
...
pt = 'D:\\worldforge\\assets\\dural\\creatures\\biped\\male\\texture\\male_body\\D.dds'
ogre_mat_file = os.path.abspath(pt)[:-5] + 'ogre.material'
dif = os.path.abspath(pt)[:-5] + 'D.dds'
spc = os.path.abspath(pt)[:-5] + 'S.dds'
nrm = os.path.abspath(pt)[:-5] + 'N.dds'
if os.path.isfile(ogre_mat_file):
	
	test =has_alpha_diffuse_map(dif)
	print test
